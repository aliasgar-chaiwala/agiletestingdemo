...
function writeToSlackJson(content, fileObj, envn) {
    if (_pack == "End2End") {
        fileObj[envn].SummaryE2E = content.summary;
...
var slackToken = 'xoxb-51113602688-I3sEaTz1bk5waEp9RZnWrNRD';

//initialize node slack upload object

var slackUpload = new NodeSlackUpload(slackToken);
var slackChannel = {
    channelHook: 'https://hooks.slack.com/services/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',
...
function pushReportToSlack(_report) {
    ...
    slackUpload.uploadFile({
        file: fs.createReadStream(path.join("C:/", _report.dest, _report.name)),
        filetype: 'html',
        title: 'ZAP Report',
        initialComment: '*' + 'Basic ZAP scan for Search Functionality ' + '*',// + slackJson[_env].Environment + ' on ' + dateFormat(currentDate, "dd/mm/yyyy") + '*' + '\nNote: Download report & open it in Browser',
