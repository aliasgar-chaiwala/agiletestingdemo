...
var generateReport = function (options) {
    ...
    //console.log('suite Features:', suite.features.summary);//.passed, suite.features[0].summary.failed,suite.features[0].summary.ambiguous,'\n');
...
function generate(options, callback) {
    ...
    if (isValidJsonFile()) {
        var results = generateReport(options);
        //console.log('results :', results, '\n');
        launchReport();
        return callback ? callback() : results;//true;
    }
}
